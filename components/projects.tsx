		...
		>
			<div className='flex flex-col border-zinc-500 rounded-lg border-2 my-2 px-2 py-3 transition ease-in-out delay-50 hover:scale-[1.01] duration-200'>
				...
				<div className='flex items-end'>
					...
					<span className='mt-8 mb-auto text-gray-400'>
						{tldr}
					</span>
						<div className='mt-4 mb-auto'>
							<span className='font-bold block'>{"What I learned: "}</span>
							<span className={`${lineClampStyles} sm:line-clamp-none text-gray-400`}>
								{description}
							</span>
							{!showMore && <button className={'sm:hidden'} type={'button'} onClick={(e) => {
								e.preventDefault();
								setShowMore(true);
							}}>See all</button>}
					</div>
